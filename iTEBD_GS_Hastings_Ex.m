...
for it1 = (1:Nstep)
    ...
    for it2 = (1:2) % 1 (2): update odd (even) bonds
        ...
        Phi = contract(expH,4,[3 4],T,4, [2 3], [3 1 2 4]);
        ...
        % can diverge!
        norm1 = norm(Lambda{it2});
        Lambda{it2} = Lambda{it2}/norm1;
        Phi = Phi/norm1;


        % Update Bs{1}, Bs{2}
        Bs{3-it2} = VT;
        Bs{it2} = contract(Phi,4,[4 3],conj(VT),3,[2 3],[1 3 2]);
...
%         if it2 == 1
%             TA = UT;
%             TB = contract(diag(Lambda{it2}),2,2,VT,3,1);
%             TC = Bs{1};
%         else
%             TA = contract(diag(Lambda{2}),2,2,Bs{1},3,1);
%             TB = contract(UT,3,2,diag(Lambda{it2}),2,1,[1 3 2]);
%             TC = VT;
%         end
