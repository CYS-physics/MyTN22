...
for itN = (1:N)
    ...
    % 1-site term
    PH = contract(Hlr{itN},3,2,M{itN},3,1);
    PH = contract(PH,4,[2 4],Hs{itN},4,[3 2]);
    PH = contract(PH,4,2,diag(S),2,1);
    PH1 = contract(PH,4,[3 4],Hlr{itN+2},3,[3 2]);

    PL = contract(conj(M{itN}),3,2,M{itN},3,2);

    PH1 = PH1 - contract(PH1,3,[1 2],PL,4,[1 2], [2 3 1]);

    varE1(itN) = contract(PH1,3,(1:3),PH1,3,(1:3));

    if itN<N
        % 2-site term
        PH2 = contract(PH,4,4,M{itN+1},3,1);
        PH2 = contract(PH2,5,[3 5],Hs{itN+1},4,[3 2]);
        PH2 = contract(PH2,5,[3 5],Hlr{itN+3},3,[2 3]);

        PR = contract(M{itN+1},3,1,conj(M{itN+1}),3,1);
        PH2 = PH2 - contract(PH2,4,[1 2],PL,4,[1 2], [3 4 1 2]);
        PH2 = PH2 - contract(PH2,4,[3 4],PR,4,[2 1], [1 2 4 3]);


        varE2(itN) = contract(PH2,4,(1:4),PH2,4,(1:4));


        % update MPS canonical index
        M{itN+1} = contract(diag(S),2,2,M{itN+1},3,1);
        [M{itN+1},S] = svdTr(M{itN+1},3,[1 3],[],[]);
        M{itN+1} = permute(M{itN+1}, [1 3 2]);


    end

    % update left side of Hlr
